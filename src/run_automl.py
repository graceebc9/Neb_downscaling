...
def run_automl(df_path, pc_path , config, output_directory, test_size, time_limit=1000):
    ...
    if config.region_type == 'ldcd_seperate':
        train_data = load_data(config, df_path, pc_path, 'y')
        test_data = load_data(config, df_path, pc_path, 'n')
    else:
        data = load_data(config, df_path, pc_path)
